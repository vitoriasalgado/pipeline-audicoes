...
- **Camadas medalhão separadas por responsabilidade:** a extração só escreve o `raw` (JSON cru) e NÃO se mistura com transformação; a transformação lê `raw` e escreve Parquet em `processed`; a carga modela e carrega o Postgres. A lógica de cada etapa vive **inline dentro das DAGs** — elas não importam nada de `lastfm/` nem de `spotify/`. A exceção são os módulos vizinhos dentro da própria `dags/`, que **não importam Airflow** e por isso podem ser importados e testados no host: `alertas.py`, `validacoes.py` e `transformacoes.py` (o miolo puro da prata do Last.fm, extraído para virar testável). Etapa nova que tenha lógica sem I/O pode seguir esse padrão; o que não pode é a DAG depender das pastas por fonte. **O que roda no Airflow é sempre o código em `dags/`.** As pastas por fonte guardam scripts de host (rodados à mão, com `localhost`) e o registro de como cada etapa foi construída; quando um deles deixa de refletir a DAG, vai para `arquivo/` em vez de ficar dando a impressão de ser a etapa atual.
...
- **Modelo estrela/constelação:** duas fatos (`fato_audicoes` do Last.fm, `fato_top_spotify`) compartilhando `dim_artista` e `dim_faixa`. A `dim_tempo` é **só da `fato_audicoes`** — a do Spotify guarda `snapshot_date` como coluna, porque o grão da dimensão é hora cheia e uma coleta semanal não tem hora significativa. Assimetria deliberada, registrada na §5 do PRD; não "consertar". `spotify_*_id` e `mbid` são atributos de apoio e podem vir vazios.
...
- **Nenhum `DO UPDATE` atribui `EXCLUDED` direto.** A regra é `COALESCE(NULLIF(<existente>, ''), NULLIF(<novo>, ''))` nos três pontos que atualizam atributo (`mbid` e `album` no Last.fm, `album` no Spotify): o primeiro valor não-vazio vence, com preferência pelo que já está no banco. O `NULLIF` é obrigatório nos **dois** lados — sem ele no novo, o `''` que o Last.fm devolve em parte dos scrobbles apaga o valor bom (foi o bug de ago/2026, 70 mbids perdidos e recuperados do bronze); sem ele no existente, um `''` já gravado trava o campo para sempre, porque `''` não é `NULL`. Motivo completo na §6 do PRD.
...
Estrutura das pastas: `dags/` (as **duas** DAGs — `pipeline_audicoes` e `pipeline_spotify`; o compose monta essa pasta, e é o que roda), `lastfm/` e `spotify/` (um utilitário de host cada: `ler_parquet.py` e `test_spotify.py`), `db/` (`schema.sql` + `migracoes/` + `consultas/`, as queries analíticas das missões 12 e 18 — **as duas do cruzamento não são duplicata**: uma é `LEFT JOIN` recortado no top 20, que cabe numa tela, a outra é `FULL OUTER` sem recorte, que mostra as três regiões; o cabeçalho de cada uma declara a pergunta que responde), `tests/` (testes da transformação, rodados no host), `scripts/` (backfill e o reparo pontual de dimensões; também montada nos containers), `arquivo/` (código aposentado mantido como registro — as primeiras missões e as versões de etapa que a DAG deixou para trás).
...
Há testes só da transformação, em `tests/` — `pytest` a partir da raiz, na venv do host; rodam em segundos e não sobem Airflow, MinIO nem Postgres. Não há linter. Fora isso, a "verificação" de cada missão é o checkpoint visível descrito no roteiro (saída no terminal, arquivo no bucket, task verde no Airflow). Não existe teste dos upserts: exigiria um Postgres real, e a decisão de não pagar por isso está na §9.1 do PRD.
